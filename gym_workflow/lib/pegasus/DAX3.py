...
class Metadata:
	...
	def __unicode__(self):
		return "<Metadata %s %s = %s>" % (self.type, self.key, self.value)
	...
	def getArguments(self):
		...
		for a in self.arguments:
			if isinstance(a, File):
				args.append(str(a.toArgumentXML()))
			else:
				args.append(a)
		return ''.join(args)
...
class Job(AbstractJob):
	...
	def __unicode__(self):
		return "<Job %s %s::%s:%s>" % (self.id, self.namespace, self.name, self.version)
...
class DAX(AbstractJob):
	...
	def __init__(self, file, id=None, node_label=None):
		...
		if isinstance(file, File):
			self.file = file
		elif isinstance(file, str) or isinstance(file, str):
